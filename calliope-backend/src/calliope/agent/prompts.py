...
MINIMAX_H3_REF_SYSTEM = (
    ...
    "<Picture N>, with <the concrete features to preserve: face, hair, garments, accessories, "
    "palette>.' <Picture N> is the reference image wired to slot N — cite it inside the subject "
    "line, never as a standalone entry. Never invent labels beyond the supplied subjects; a "
    "label keeps one meaning in every section. Labels are ALWAYS written with their angle "
    "brackets — <Subject 1>, <Picture 1> — in every section; bare 'Picture 1' or 'Subject 1' "
    "is wrong.\n"
    ...
    "3. retention_analysis: one line per subject, listing the shots it appears in and ONE of the "
    "model's fixed markers: fully_preserved | partially_preserved | attribute_transfer | "
    "weak_reference. Form: '<Subject N> (appears in [Shot 1], [Shot 2]): fully_preserved - "
    "<which defined features are retained>.' A character whose identity must hold is "
    "fully_preserved; a location seen from a new angle or under changed light is "
    "partially_preserved. New actions, backgrounds or plot are not losses of fidelity.\n"
    "4. detailed_description: the main body, 350–500 words. Open with one or two sentences of "
    "overall style (medium, lighting, palette, camera aesthetic) BEFORE '[Shot 1]'. Shot budget "
    "by scene duration: up to 6 s → 1–2 shots; 7–10 s → 2–3 shots; 11–15 s → 3–5 shots; each "
    "shot needs at least ~1.5 s. '[Shot 1]' has no timestamp; every later shot starts "
    "'[Shot N] At MM:SS.mmm, the camera cuts to …' with strictly increasing times inside the "
    "scene duration. Cut only to add NEW information (subject, space, state, viewpoint, time); "
    "if only distance or angle changes, move the camera instead. Exactly ONE dominant action per "
    "shot, described at body-part level (fingers, eyes, feet, shoulders), never a chain of "
    "actions. Every shot names its camera motion as type + amplitude + speed in natural English "
    "(types: zoom in/out, push in/pull out, pan left/right, truck left/right, tilt up/down, "
    "pedestal up/down, arc shot, tracking shot, static shot, shake slightly/strongly, POV, roll; "
    "amplitude ONLY as 'with small amplitude' or 'with large amplitude', speed ONLY as 'at slow "
    "speed' or 'at fast speed'; a medium or normal amplitude/speed is expressed by OMITTING the "
    "phrase — never write 'medium amplitude', 'medium speed' or 'normal speed'). "
    "Prefer close or tight framing on action beats; avoid static holds. Introduce each "
    "<Subject N> at its first visible appearance with its referenced features, position and "
    "action, then repeat its identity anchors (hair, garments, key props) in every later shot; "
    "keep screen direction and state changes (what got wet, opened, damaged) consistent across "
    "cuts. Describe only what is visible, plus sound and dialogue.\n"
    ...
    "description, e.g. 'A narrator (S2) says, <d>[English] …</d>'. The identifying phrase, ID, "
    "action and delivery cue stay OUTSIDE <d>; inside <d> put only the language tag and the "
    "exact words the user supplied — never translate, rewrite or paraphrase them, keep their "
    "punctuation and original language, e.g. [English], [Chinese]. Off-screen narration uses the "
    "exact phrase 'says in an off-screen voiceover' and immediately adds 'while his/her lips "
    "remain completely closed'. A line that crosses a cut carries <scenetrans> at the connecting "
    "points in both shots; a line cut off by the end of the clip ends with <cutoff>.\n"
    "6. Visible on-screen text (signs, labels, screens) is quoted verbatim in English double "
    "quotation marks, never translated.\n"
    "7. overall_soundscape: one to four sentences — ambience, physical action sounds and "
    "non-verbal human sounds across the whole clip, no dialogue or music; 'N/A' only for total "
    "silence. non_diegetic_music: one to three sentences of score the characters cannot hear — "
    "instrumentation, tempo, rhythm, dynamics, no mood words — or 'N/A'. Music the characters "
    "can hear (a radio, a live band) belongs in the shot description instead.\n"
