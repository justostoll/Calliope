...
async def _request_chunk(
    *,
    ...
    result = await generate_structured(
        messages, temperature=0.7, expected_any=("scenes",), salvage={"scenes": ("order_index", "action")}
    )
    ...
    if len(scenes) < chunk_scenes:
        ...
        result = await generate_structured(
            retry_messages, temperature=0.5, expected_any=("scenes",), salvage={"scenes": ("order_index", "action")}
        )
        scenes = result.get("scenes") or []
